...
describe('page-nav block', () => {
  ...
  // The per-component status card is built by blocks/component-status and mounted
  // by page-nav at the top of the rail, above the table of contents.
  describe('component status card mount', () => {
    const STATUS_INDEX = {
      implementations: { web: [{ id: 'figma', label: 'Figma' }, { id: 'rsp', label: 'React Spectrum' }] },
      components: [{
        name: 'ActionButton',
        label: 'Action Button',
        platforms: { web: { figma: { status: 'available' }, rsp: { status: 'experimental' } } },
      }],
    };
    let originalUrl;
    ...
    beforeEach(() => {
      ...
      sandbox.stub(window, 'fetch').callsFake((url) => {
        if (String(url).includes('status-index.json')) {
          return Promise.resolve(new Response(JSON.stringify(STATUS_INDEX), { status: 200 }));
        }
        // widgets fragment (and anything else): an empty fragment.
        return Promise.resolve({ ok: true, text: async () => '<body><main><div></div></main></body>' });
      });
      makeDOM();
    });
    ...
    it('mounts the status card as the first child, above the TOC, on a component page', async () => {
      window.history.pushState({}, '', '/web/rsp/components/action-button');
      await init(el);

      const card = el.querySelector('.component-status');
      expect(card).to.not.be.null;
      expect(el.firstElementChild).to.equal(card);
      // the TOC list follows the card
      expect(card.nextElementSibling).to.equal(el.querySelector('ul'));
      expect(card.querySelectorAll('.component-status-row').length).to.equal(2);
    });

    it('does not mount a status card on a non-component interior page', async () => {
