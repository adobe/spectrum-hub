 ...
 *
 * Two fields, deliberately, because they answer different questions and disagree for
 * real components:
 *
 *   docs    which page the source's own documentation site publishes it on. Several
 *           components can share one page — AlertDialog, Dialog and FullscreenDialog
 *           all link to Dialog.html — so this is many-to-one.
 *   export  what to import and render. One-to-one: FullscreenDialog is its own real
 *           component even though its docs redirect to Dialog's page.
 *
 * They differ today on `cards` (Card vs CardView), `radio-button` (RadioGroup vs Radio)
 * and `takeover-dialog` (Dialog vs FullscreenDialog). Collapsing them to one value has
 * shipped a bug before — the playground 404'd rendering components whose docs merely
 * redirected elsewhere. Keep both, and read the one that answers your question.
 *
 * `docs` comes from each cell's `upstreamName` (auto-derived, or a manual override —
 * see applyOverrides), read off the already-computed slices so a `page` override's
 * shared slug is picked up for free. `export` comes from the roster, where an entry
 * exists only when the implementation's own name differs from the canonical one.
 *
 * The shape is implementation-agnostic, but only RSP populates `export`: SWC renders by
 * tag name, which `deps/swc/components.json` already maps, so it has no separate export
 * to record. Figma is excluded entirely — its redirect field is `figmaPageSource`, not a
 * code implementation anything looks up here.
 *
 * Written to `deps/impl-component-names.js` as a plain `export default {...}` module
 * rather than JSON: this repo's ESLint parser target cannot follow a JSON module through
 * its consumers' imports, and a plain object literal needs no special loading in the
 * browser. It is a couple dozen entries at most, so shipping it as a module avoids a
 * per-page network round trip for data that is usually empty for the current page.
 *
