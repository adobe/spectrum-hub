...
/**
 * Canonical slug -> real RSP export name, for every roster member whose real RSP export
 * name differs from its own canonical name (e.g. canonical "ActionGroup" ships as RSP's
 * `ActionButtonGroup`).
 *
 * This answers a DIFFERENT question than buildImplAliases' originalName: that table answers
 * "what does react-spectrum.adobe.com call this on its docs page" — several distinct real
 * components can share one doc page (AlertDialog, Dialog, and FullscreenDialog all link to
 * Dialog.html; ColorHandle and ColorLoupe share a manually-overridden `color-handle-and-loupe`
 * page slug) — while this table answers "what do I actually `import` and instantiate."
 * AlertDialog is still its own real, separate RSP component distinct from Dialog even though
 * its docs redirect there, so reusing originalName for rendering would silently swap it (and
 * FullscreenDialog, CloseButton) for the generic component its docs happen to point at.
 *
 * Built from `sources.rsp` — the real RSP roster key `joinRosters` joined into this canonical
 * name — rather than from any alias's `externalName`, so a manually-overridden originalName
 * (like ColorHandle/ColorLoupe's, which don't correspond to any real standalone RSP export at
 * all) never leaks into what the playground tries to render.
 *
 * @param {{ name: string, sources: Record<string, string> }[]} roster
...
function main() {
  ...
  const rspExportNamesModule = '// Generated by deps/build-status-index.js — do not edit by hand.\n'
    + `export default ${JSON.stringify(rspExportNames, null, 2)};\n`;
  writeFileSync(RSP_EXPORT_NAMES_FILE, rspExportNamesModule);
