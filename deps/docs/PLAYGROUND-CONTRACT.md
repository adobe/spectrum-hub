...
Overrides are declared only where the names actually diverge — 13 of 121 RSP components — in the generated [`deps/rsp-export-names.js`](../rsp-export-names.js) (`action-group` → `ActionButtonGroup`, `table` → `TableView`).
...
| What do I import and render? | `deps/rsp-export-names.js` |
| Where does the public doc page live? | `upstreamName`, in the alias and override files |
