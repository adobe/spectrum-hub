...
Component-specific option overrides cover documented presets for an otherwise open-ended type. RSP Avatar is the current case: it accepts arbitrary numbers and `<number>lh` strings, so its catalog correctly has no closed `values` list, but the playground picker still offers the documented numeric sizes. Keep these overrides scoped by implementation, component, and property so they cannot leak into unrelated controls that share a property name.
