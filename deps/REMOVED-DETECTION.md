...
Spike deliverable for the "Removed" detection story. It recommends how Spectrum Hub should detect that a component has disappeared from an implementation and populate the **Removed** status reliably, without false positives.

No implementation emits a "removed" record — a removed component simply stops appearing. Detection is therefore a **diff over time**, owned by spectrum-hub. This work is **deferred and not required for the initial launch**; Removed can be set by hand through [status-overrides.json](./status-overrides.json) (the manual override story) on day one, and this automatic detection augments that later.

This spike depends on the aggregate index existing first — see the index build story. It complements the data-contract spike in [DATA-CONTRACT.md](./DATA-CONTRACT.md) and the unified model in [scripts/utils/status-model.js](../scripts/utils/status-model.js).
...
### Spectrum Web Components (SWC) — no automatic membership signal

- `deps/swc/components.json` is a **manually maintained flat array of tags**. There is no discovery script. Components only leave the list when a person edits it.
- [extract-cem-components.js](./swc/extract-cem-components.js) iterates that allow list. If a tag is missing from the CEM it **warns and skips** (`Warning: <tag> not found in CEM`), leaving the previous data file in place. A component vanishing from the published CEM therefore produces a warning and a stale file, **not** a membership drop.
- Consequently, an SWC removal surfaces only as (a) a manual allow-list edit or (b) a diff of the CEM declaration set. Automatic SWC removal detection is lower value until the CEM is published and wired to a discovery step of its own.
...
- The daily bot commits with a fixed message (`chore: update React Spectrum component properties`). **Git history is therefore a free, per-run, auditable snapshot trail.**
- `deps/status-index.json` and its builder do not exist yet. This recommendation specifies behavior to build **into** that builder.
...
**Hard prerequisite:** the aggregate index and its builder (`deps/build-status-index.js`) must exist first. This story cannot start before the index build story lands.
...
**Estimate: one story, ~M**, assuming the index builder already exists. The detection logic is a contained state machine plus guards; most of the effort is the fixtures and edge-case tests. If SWC needs its own discovery step to gain a membership signal, split that out separately — it is not in scope here.
...
- Harden `discover-components.js` against a **partial/truncated** `?meta` response (e.g. sanity-check the count against the prior run) so silent under-discovery cannot masquerade as mass removal upstream of the index.
- SWC has no automatic membership signal. Decide whether SWC removal detection waits on CEM publication plus a discovery step, or stays manual (override-only) for now.
